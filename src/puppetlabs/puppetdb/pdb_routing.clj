...
(defn resource-request-middleware [req]
  (resource-request req "public"))

(defn pdb-core-routes [shared-globals submit-command-fn query-fn enqueue-raw-command-fn]
  (map (fn [[uri route]]
         (compojure/context uri [] route))
       (partition 2 ["/meta" (meta/build-app shared-globals)
                     "/cmd" (cmd/command-app shared-globals enqueue-raw-command-fn)
...
(tk/defservice pdb-routing-service
  ...
  (start [this context]
         (add-ring-handler this (pdb-app (get-route this)
                                         (pdb-core-routes (shared-globals) submit-command query enqueue-raw-command)))
         context))

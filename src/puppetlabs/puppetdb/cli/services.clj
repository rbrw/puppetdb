...
(defn start-puppetdb
  ...
  (let [{:keys [database developer read-database emit-cmd-events?]} config
    ...
    (when (> 1 (count (conf/reduce-section conj database [])))
      (throw
        ...
        (let [stockdir (conf/stockpile-dir config)
          ...
          (let [job-pool (mk-pool)
                ...
                write-dbs (conf/reduce-section
                           ...
                           []
                           database)
                write-db-names (conf/reduce-section (fn [result name settings]
                                                      (conj result name))
                                                    [] database)
                [write-db] write-dbs]
    ...
    (catch ExceptionInfo ex
      (let [{:keys [kind] :as data} (ex-data ex)
        ...
        (case kind
          ...
          ::must-migrate-multiple-databases (stop (.getMessage ex) 2)

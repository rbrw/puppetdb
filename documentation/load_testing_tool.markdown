...
the one below:
    ...
    $ java -cp /opt/puppetlabs/server/apps/puppetdb/puppetdb.jar clojure.main -m puppetlabs.puppetdb.cli.benchmark --config myconfig.ini --catalogs /tmp/puppetdb-bak/catalogs --runinterval 30 --numhosts 1000 --rand-perc 10

Note that if you run it via leiningen, you should make sure to use
trampoline, i.e. `lein trampoline run benchmark ...` so that the tool
can shut down and clean up normally.

If you have `eatmydata` available, the current implementation should
run substantially faster under its control, e.g.:

    $ eatmydata java -cp /opt/puppetlabs/server/apps/puppetdb/puppetdb.jar ...

But `eatmydata` should *not* be used for non-benchmark PuppetDB invocations.

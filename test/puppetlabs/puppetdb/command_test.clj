  ...
  (deftest deactivate-node-node-missing
    (testing "should add the node and deactivate it"
      (with-test-db'
        (doseq [{:keys [certname command]} cases]
          (test-msg-handler command publish discard-dir
            (let [result (first (query-to-vec "SELECT certname, deactivated
                                                 FROM certnames"))]
              (is (= (:certname result) certname))
              (is (instance? java.sql.Timestamp (:deactivated result)))
...
(deftest store-v6-report-test
  (let [v6-report (-> v5-report
    ...
    (with-test-db'
      (test-msg-handler command publish discard-dir
        (is (= (query-to-vec
                "select certname, configuration_version, environment_id
                   from reports")
    ...
    (with-test-db'
      (test-msg-handler command publish discard-dir
        (is (= (query-to-vec
                "select certname, configuration_version, environment_id
                   from reports")
               [(with-env (select-keys v5-report
                                       [:certname :configuration_version]))]))
        (is (= 0 (times-called publish)))
        (is (empty? (fs/list-dir discard-dir)))))))
  ...
  (let [command {:command store-report
    ...
    (with-test-db'
      (test-msg-handler command publish discard-dir
        ...
        (is (= "unchanged" (-> (query-to-vec
                                "select rs.status from reports r

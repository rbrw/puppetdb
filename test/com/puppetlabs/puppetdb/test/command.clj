...
(deftest fact-path-update-issue
  ;; Simulates two commands being processed for two different machines
  ;; at the same time
  (let [certname-1 "some_certname1"
        ...
        ;; I'm modifying delete-certname-facts! so that I can coordinate access
        ;; between the two threads, I'm storing the reference to the
        ;; original delete-certname-facts! here, so that I can
        ;; delegate to it once I'm done coordinating
        storage-delete-certname-facts! scf-store/delete-certname-facts!]
    ...
    (with-redefs [scf-store/delete-certname-facts!
                  (fn [factset fact-ids]
                    ;; (println
                    ;;  "In the hacked delete-certname-facts, counting down - "
                    ;;  (thread-id))
                    ;; (flush)
                    ...
                    (flush)
                    ;; Execute the normal delete-certname-facts!
                    ;; function (unchanged)
                    (storage-delete-certname-facts! factset fact-ids))]
      (let [first-message? (atom false)
            second-message? (atom false)
            fut-1 (future
                    (test-msg-handler command-1b publish discard-dir
                      (reset! first-message? true)))
            ;; _ (do (println "future 1 launched") (flush))
            fut-2 (future
                    (test-msg-handler command-2b publish discard-dir
                      ;;(println "in the message handler?????")
                      (reset! second-message? true)))
            ;;_ (do (println "future 2 launched") (flush))
            ]
